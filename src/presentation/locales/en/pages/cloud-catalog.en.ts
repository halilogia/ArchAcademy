...
const CloudCatalogPage = () => {
  ...
  const items: ArchItem[] = [
    { id: 'gitops', name: 'GitOps & IaC', path: '/gitops', color: '#f97316', icon: <GitBranch size={32} />, desc: 'Manage infra and ops processes via code (Git).', gridArea: 'a' },
    { id: 'container', name: 'Container Arch', path: '/containerization', color: '#06b6d4', icon: <Container size={32} />, desc: 'Docker containers and Kubernetes orchestration.', gridArea: 'b' },
    { id: 'serverless', name: 'Serverless (FaaS)', path: '/serverless', color: '#f59e0b', icon: <Zap size={32} />, desc: 'Code functions only, no server management.', gridArea: 'c' },
    { id: 'microservices', name: 'Microservices', path: '/microservices', color: '#3b82f6', icon: <Share2 size={32} />, desc: 'Break monolith into independent, small, autonomous services.', gridArea: 'd' },
    { id: 'zerotrust', name: 'Zero Trust', path: '/zero-trust', color: '#ef4444', icon: <ShieldCheck size={32} />, desc: 'Trust no device inside network; verify every request.', gridArea: 'e' },
    { id: 'soa', name: 'SOA Arch', path: '/soa', color: '#8b5cf6', icon: <Globe size={32} />, desc: 'Enterprise Service-Oriented Architecture (Microservices predecessor).', gridArea: 'f' },
    { id: 'spacebased', name: 'Space-Based', path: '/space-based', color: '#ec4899', icon: <Server size={32} />, desc: 'In-memory data grids for extreme scale.', gridArea: 'g' },
    { id: 'bff', name: 'BFF Pattern', path: '/bff', color: '#3b82f6', icon: <Share2 size={32} />, desc: 'Dedicated backend layer managed by frontend teams.', gridArea: 'i' }
  ];
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ background: '#0f172a', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
      ...
      <div className="container" style={{ position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
           ...
           <p style={{ color: '#94a3b8', fontSize: '1.2rem' }}>Modern infra and ops architectures</p>
        </div>
