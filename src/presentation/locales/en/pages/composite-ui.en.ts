  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ background: '#020617', minHeight: '100vh', paddingBottom: '100px' }}>
      <ArchHero 
        ...
        description="Interface fragments (Micro-Frontends) developed by different teams and hosted on different servers seamlessly fused within a main shell (Shell)."
        badge="Enterprise Architecture"
        color="#8b5cf6"
        illustration={illustration}
        features={[
          { icon: <LayoutDashboard />, title: 'Shell Application', desc: 'Main skeleton of the application. Provides shared services such as navigation and shared state.' },
          { icon: <Share2 />, title: 'Fragments', desc: 'Small standalone UI islands. Can be a dashboard card or an entire page.' },
          { icon: <Grid />, title: 'Runtime Integration', desc: 'Components load at runtime (JS/Module Federation) instead of build time.' }
        ]}
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem' }}>
          <div className="glass-card" style={{ borderLeft: '4px solid #8b5cf6' }}>
            <h3 style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '1.5rem', color: '#c084fc' }}>
              <Layers /> Micro-Frontends Relationship
            </h3>
            <p style={{ color: '#94a3b8', lineHeight: 1.7 }}>
              Composite UI is the visual and structural counterpart of the Micro-Frontends architecture. It can be thought of as the projection of backend microservices onto the user interface. Each team owns its own fragment.
            </p>
          </div>
          <div className="glass-card" style={{ borderLeft: '4px solid #8b5cf6' }}>
            <h3 style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '1.5rem', color: '#c084fc' }}>
              <ShieldCheck /> Separation of Responsibilities
            </h3>
            <p style={{ color: '#94a3b8', lineHeight: 1.7 }}>
              The main shell application prevents fragments from breaking each other. Cross-cutting concerns such as security, authentication, and logging are centralized in the Shell.
            </p>
          </div>
        </div>
      </div>
