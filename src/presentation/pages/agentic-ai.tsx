      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ background: 'var(--bg-dark)', minHeight: '100vh' }}>
        ...
        <div className="container" style={{ marginTop: '2rem' }}>
          <AnimatePresence mode="wait">
            {activeTab === 'patterns' && <AgenticCorePatternsTab key="patterns" />}
            {activeTab === 'orchestration' && <MultiAgentOrchestrationTab key="orchestration" />}
            {activeTab === 'simulation' && <AgentSwarmSimulationTab key="simulation" />}
          </AnimatePresence>
        </div>
